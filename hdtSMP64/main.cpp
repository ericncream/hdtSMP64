...
namespace hdt
{
	...
	bool SMPDebug_Execute(void* paramInfo, void* scriptData, TESObjectREFR* thisObj,
	                      void* containingObj, void* scriptObj, void* locals, double* result,
	                      void* opcodeOffsetPtr)
	{
		...
		if (_strnicmp(buffer, "dumptree", MAX_PATH) == 0)
		{
			if (thisObj)
			{
				...
				DumpNodeChildren(thisObj->unkF0->rootNode);
			}
			else
			{
